...
func _process(delta):
	...
	if(transform.origin.y < 10):
		Rig.add_constant_force(Vector3.UP)
...
func _Seperation():
	...
	Rig.linear_velocity = Rig.linear_velocity + Vector3(close_dx,close_dy,close_dz)*Avoid_Factor
...
func _Alignment():
	...
	if(Neighboring_boids > 0):
		Rig.linear_velocity = Rig.linear_velocity + (Vector3((xvel_avg / Neighboring_boids),(yvel_avg / Neighboring_boids),(zvel_avg / Neighboring_boids)) - Rig.linear_velocity) * Matching_factor
...
func _Cohesion():
	...
	Rig.linear_velocity = Rig.linear_velocity + (avg_pos - transform.origin)*Centering_factor

func _RandoDirect():
	Rig.linear_velocity = Rig.linear_velocity + Vector3(randf_range(-1,1),randf_range(-1,1),randf_range(-1,1))*rand_factor
	pass
